...
export function ProductsClient({ initialProducts: products }: ProductsClientProps) {
  ...
  return (
    <div className="min-h-screen pb-28" style={{ backgroundColor: "#FFF8F3", fontFamily: "Hanken Grotesk, sans-serif" }}>
      <div className="max-w-6xl mx-auto px-5 md:px-10 py-8 md:py-12">
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-28 text-center rounded-2xl"
            ...
            <h3 className="text-xl mb-2" style={{ fontFamily: "Libre Caslon Text, serif", color: "#201A14" }}>Belum ada koleksi</h3>
            <p className="text-sm max-w-xs mb-8" style={{ color: "#867462" }}>Mulai tambah produk pertamamu. Unggah foto terbaik dan isi detail produk.</p>
            <button onClick={() => setAddOpen(true)} className="flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold transition-all hover:brightness-95 active:scale-[0.97]"
              style={{ backgroundColor: "#6B4E2A", color: "#FFFFFF" }}>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm" style={{ color: "#867462" }}>
              Tidak ada koleksi untuk <span style={{ color: "#201A14" }}>&ldquo;{search}&rdquo;</span>
            </p>
            <button onClick={() => setSearch("")} className="text-sm mt-3 hover:underline" style={{ color: "#6B4E2A" }}>Hapus pencarian</button>
          </div>
        ) : view === "grid" ? (
          <>
      ...
      {editProduct && <ProductFormModal open={true} onClose={() => setEditProduct(null)} mode="edit" product={editProduct} />}
    </div>
  );
}
